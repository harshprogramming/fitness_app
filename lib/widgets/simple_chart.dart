...
class SimpleBarChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AspectRatio(
      aspectRatio: 16 / 9,
      child: CustomPaint(painter: _BarPainter(values, labels, maxVal)),
    );
  }
}
...
class _BarPainter extends CustomPainter {
  ...
  void paint(Canvas c, Size s) {
    ...
    final bw = s.width / (values.length * 2 + 1);
    final tp = TextPainter(textDirection: TextDirection.ltr);

    for (var i = 0; i < values.length; i++) {
      final x = bw * (1 + i * 2);
      final h = (values[i] / maxVal) * (s.height * 0.75);
      final r = Rect.fromLTWH(x, s.height - h - 20, bw, h);
      p.color = Colors.blueGrey;
      c.drawRect(r, p);

      tp.text = TextSpan(text: values[i].toStringAsFixed(0), style: const TextStyle(fontSize: 10, color: Colors.black87));
      tp.layout();
      tp.paint(c, Offset(x, s.height - h - 35));

      if (labels != null && i < labels!.length) {
        tp.text = TextSpan(text: labels![i], style: const TextStyle(fontSize: 10, color: Colors.black54));
        tp.layout(maxWidth: bw * 2);
        tp.paint(c, Offset(x - bw * 0.3, s.height - 16));
      }
    }
  }
