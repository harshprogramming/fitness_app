...
class CaloriesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final app = AppScope.of(context);
    final total = app.caloriesToday;
    final remaining = (app.dailyCalorieGoal - total).clamp(0, 99999);
    final meals = app.mealsForSelectedDate;

    void addFor(MealType t) {
      showModalBottomSheet(
        context: context,
        showDragHandle: true,
        isScrollControlled: true,
        builder: (ctx) => _AddFoodSheet(type: t),
      );
    }
    ...
    return Scaffold(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () async {
              ...
              if (d != null) await app.setSelectedDate(d);
            },
          )
        ],
      ),
      body: app.loading && !app.bootstrapped
        ...
        children: [
          Card(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text('Daily Goal: ${app.dailyCalorieGoal} kcal'),
                  const SizedBox(height: 8),
                  Text('Consumed: $total kcal   •   Remaining: $remaining kcal'),
                ],
              ),
          ...
          _section(context, 'Breakfast', MealType.breakfast, meals, addFor, (id) async => await app.removeMeal(id)),
          _section(context, 'Lunch', MealType.lunch, meals, addFor, (id) async => await app.removeMeal(id)),
          _section(context, 'Dinner', MealType.dinner, meals, addFor, (id) async => await app.removeMeal(id)),
          _section(context, 'Snacks', MealType.snacks, meals, addFor, (id) async => await app.removeMeal(id)),
        ],
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: () => addFor(MealType.snacks),
        ...
        child: Column(
          ...
          children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              IconButton(icon: const Icon(Icons.add), onPressed: () => onAdd(type)),
            ]),
            ...items.map((e) => ListTile(
              dense: true,
              title: Text(e.name),
              subtitle: e.serving != null ? Text(e.serving!) : null,
              trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                Text('${e.calories} kcal'),
                IconButton(icon: const Icon(Icons.delete_outline), onPressed: () => onRemove(e.id)),
              ]),
            )),
            if (items.isEmpty)
              Padding(
                padding: const EdgeInsets.only(top: 8),
                child: Text('No items', style: Theme.of(context).textTheme.bodySmall),
              ),
          ],
        ),
      ),
...
class _AddFoodSheet extends StatefulWidget {
  const _AddFoodSheet({required this.type});
  final MealType type;
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            TextFormField(
              controller: _serv,
              decoration: const InputDecoration(labelText: 'Serving size (optional)'),
            ),
            const SizedBox(height: 12),
            Align(
              ...
              child: ElevatedButton(
                onPressed: () async {
                  if (!_formKey.currentState!.validate()) return;
                  await app.addMeal(MealEntry(
                    id: '',
                    date: app.selectedDate,
                    mealType: widget.type,
                    name: _name.text.trim(),
                    calories: int.parse(_cal.text.trim()),
                    serving: _serv.text.trim().isEmpty ? null : _serv.text.trim(),
                  ));
